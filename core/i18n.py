...
def _get_lang_dir():
    """
    获取语言文件目录
    支持开发环境和 PyInstaller 打包后的运行环境
    """
    # 1. 如果是 PyInstaller 打包的运行程序
    if getattr(sys, 'frozen', False):
        # sys._MEIPASS 是 PyInstaller 的临时解压目录
        meipass = getattr(sys, '_MEIPASS', None)
        if meipass:
            lang_path = Path(meipass) / 'lang'
            if lang_path.exists():
                return lang_path
    
    # 2. 开发环境：相对于 i18n.py 的位置
    #    i18n.py 位于 flickr_downloader/core/i18n.py
    #    lang 目录位于 flickr_downloader/lang/
    dev_lang_dir = Path(__file__).parent.parent / 'lang'
    if dev_lang_dir.exists():
        return dev_lang_dir
    
    # 3. 尝试相对于当前工作目录
    cwd_lang_dir = Path.cwd() / 'lang'
    if cwd_lang_dir.exists():
        return cwd_lang_dir
    
    # 4. 返回默认路径（即使不存在，代码会处理）
    return dev_lang_dir
